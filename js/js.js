...
.then(function (json) {
    document.querySelector('#stus').innerHTML = json.online ? 'Online' : 'Offline';
        ...
        document.querySelector('#motd').innerHTML = json.description ? json.description.text.replace(/\§4/gi, ' ').replace(/\§a/gi, ' ').replace(/\§d/gi, ' ').replace(/\§l/gi, ' ').replace(/\§2/gi, ' ').replace(/\§f/gi, ' ').replace(/\§6/, ' ') : '';
